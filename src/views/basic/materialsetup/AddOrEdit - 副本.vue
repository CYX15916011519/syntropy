        ...
        <a-tabs :defaultActiveKey="1">
          <a-tab-pane :tab="'基本信息'" :key="1">
            ...
            <a-form-item
              ...
              :wrapper-col="formItemLayout.wrapperCol"
            >
              <a-checkbox :value="form.enable"></a-checkbox>
            </a-form-item>
          </a-tab-pane>
          <a-tab-pane v-for="tmp in Object.keys(getK3Form)" :tab="getK3Form[tmp].name" :key="getK3Form[tmp].key">
            <a-form-item v-for="(item) in getK3Form[tmp].list" :key="item.id" :label="(item.required ? '* ':'') + item.name" :label-col="formItemLayout.labelCol" :wrapper-col="formItemLayout.wrapperCol">
              ...
              <a-checkbox v-if="item.type === 'bit' && tg(item)"></a-checkbox>
  ...
  methods: {
    ...
    GetDetailed (obj) {
      ...
      _this.$store.dispatch('materialTemplateGet', obj).then(() => {
        ...
        if (result.success) {
          ...
          _this.JsonForm = JSON.parse(_this.form.template)
          this.OnCForm()
        } else {
          _this.$notification['error']({
    ...
    OnMaterialGet () {
      ...
      _this.$store
        ...
        .then(res => {
          this.JsonForm = this.$store.state.Material.Detailed.Data
          this.OnCForm()
        })
        .finally(f => {
          _this.loading = false
    ...
    OnCForm () {
      var _this = this
      var formRemarksGroup = this.K3From
      ...
      keys.forEach(k => {
        ...
        if (label) {
          ...
          if (!obj.Hide) {
            formRemarksGroup[obj.OnGroup].list.push({ id: k, name: (obj.name + '').trim(), type: (obj.type + '').trim(), OnGroup: obj.OnGroup, index: obj.index, api: obj.api, List: obj.List, required: obj.required, Parame: obj.Parame })
          }
